...
| UART receiver clock-lock | Hardware 8-N-1 UART on the real device presumably maintains byte-clock sync against the transmitter through a short run of framing errors (manual doesn't detail). | Three-state receiver (IDLE → DATA → LOCKED) in [src/host/modem.ts](../src/host/modem.ts) locks the bit clock to the first byte's start-bit edge and predicts each subsequent start bit at `lastStartEdge + 10 × spb`. A framing error (stop bit not mark) emits a single `(0x3F '?', erased=true)` callback at the correct byte position and either edge-resyncs on the next clean mark→space transition or clock-resyncs directly to `expectedEdge` when the stop-bit space extends through to the next start bit. Before this change, a single channel-level byte drop sent the receiver back to IDLE where the coarse start-bit scanner would often latch onto an internal data-bit transition of the *next* byte, inserting a phantom shifted byte and misaligning every subsequent Reed-Solomon codeword — the "some errors were uncorrectable" failure mode users reported when transmitting through the air between rooms. | `src/host/modem.ts` (`buildDemodCore`, `lockedNeedsEdge`); unit-tested in `src/host/modem.test.ts` | 🔧 (substitute) |
| Dropped-byte erasure in Review | Real device presumably passes a clean byte stream up to the FEC layer; no documented "?" marker. | `rxBuffer` appends a literal `?` at each erasure position (both in `src/host/main.ts` and `src/host/pair.ts`), so the operator sees the drop at the exact column where it happened in Review. On decode, `filterToBase32PreservingErasures` substitutes each `?` with `'A'` (the zero-bit base32 symbol) before base32 decoding, so codeword byte alignment is preserved and RS(255,223) treats each erasure as one of its ≤ 16 substitution errors per codeword. Erasures in the 12-character MI header are *not* recoverable — MI parsing is strict A-Z and a `?` in the header produces `InvalidMiError`, matching the real device's Appendix B `D_UNCORRECTABLE` screen. | `src/wire/Base32.ts` (`filterToBase32PreservingErasures`), `src/wire/EncryptedMessage.ts` (`parseDisplayForm`), `src/wire/WireFrame.ts` (`parseWireFrameFromDisplay`); integration coverage in `src/integration/stationPair.test.ts` | 🔧 (substitute) |
